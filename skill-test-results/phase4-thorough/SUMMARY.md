...
|---|---|---|
| `install-profilers` | `warning` | privileged install commands present but documented: warning |
| `install-isaacsim` | `warning` | sudo/package install examples are present and need approval: warning; cleanup command is documented: warning |
| `install-isaaclab` | `warning` | env removal example needs approval: warning |
| `benchmark-isaacsim` | `blocked_needs_approval` | Isaac Sim benchmark scripts available locally: blocked_missing_prereq; heavy benchmark sweeps not run: blocked_needs_approval |
| `benchmark-isaaclab` | `blocked_needs_approval` | long RL training not run: blocked_needs_approval |
| `profiling` | `pass` | - |
| `nsys-analyze` | `pass` | - |
| `nvtx-python` | `pass` | - |
| `profiling-api` | `blocked_missing_prereq` | Kit SDK build/run artifact test: blocked_missing_prereq |
...
| `diagnose-perf` | `warning` | governor changes require approval: warning |
| `perf-tuning` | `warning` | system setting changes present and approval-gated: warning; real before/after artifact test: blocked_needs_approval |
...
```json
{
  "nvidia_smi": {
    ...
    "returncode": 0,
    "stdout": "NVIDIA L40, 570.158.01, 49140 MiB, 1 MiB, 25, P8, 0x0000000000000001, 0 %, 35.26 W, 300.00 W",
    "stderr": "",
    "duration_s": 0.048
  },
  "cpu_governor": {
